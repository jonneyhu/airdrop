...
const injectL2Context = (l1Provider) => {
    ...
    provider.formatter.blockWithTransactions = (block) => {
        ...
        for (let i = 0; i < b.transactions.length; i++) {
            ...
            if (b.transactions[i].l1BlockNumber != null) {
                b.transactions[i].l1BlockNumber = parseNumber(b.transactions[i].l1BlockNumber);
            }
            b.transactions[i].l1Timestamp = block.transactions[i].l1Timestamp;
            if (b.transactions[i].l1Timestamp != null) {
                b.transactions[i].l1Timestamp = parseNumber(b.transactions[i].l1Timestamp);
            }
    ...
    provider.formatter.receipt = (receipt) => {
        ...
        r.l1GasPrice = ethers_1.BigNumber.from(receipt.l1GasPrice);
        r.l1GasUsed = ethers_1.BigNumber.from(receipt.l1GasUsed);
        r.l1Fee = ethers_1.BigNumber.from(receipt.l1Fee);
